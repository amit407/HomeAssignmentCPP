...
// Answer: The memory leak was because we allocated dynamic memory (using "new payload")
// and did not delete it when exiting the function.
// I used a shared pointer that points on the payload object,
// to manage the life cycle of the payload so it will be deleted only once, when the last consumer finishes its operation on the payload.
